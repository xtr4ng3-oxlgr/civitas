        ...
        serde_json::Value::Object(map) => {
            for (k, v) in map {
                match v {
                    ...
                    serde_json::Value::String(s) => println!("{pad}{k}: {s}"),
                    _ => println!("{pad}{k}: {v}"),
                }
            }
        }
...
fn print_payload<T: Serialize>(payload: &T, json_mode: bool) -> Result<()> {
    if json_mode {
        println!("{}", serde_json::to_string_pretty(payload)?);
    } else {
        print_human(&serde_json::to_value(payload)?, 0);
    }
    Ok(())
}
...
const MAX_EVIDENCE_FILES_PER_ADD: usize = 3000;
...
fn add_evidence(store: &mut Store, case_id: &str, path: &Path, copy: bool, tags: &str, actor: &str) -> Result<Vec<EvidenceRecord>> {
    ...
    if path.is_dir() {
        ...
        for e in WalkDir::new(&path).into_iter().filter_map(|x| x.ok()) {
            if e.path().is_file() {
                if count >= MAX_EVIDENCE_FILES_PER_ADD {
                    custody(store, case_id, "", actor, "evidence.add_truncated",
                        &format!("Folder scan stopped at {MAX_EVIDENCE_FILES_PER_ADD} files: {}", path.display()));
                    break;
                }
                if let Ok(x) = add_file(store, case_id, e.path(), copy, tags, actor) { out.push(x); count += 1; }
            }
        }
    }
    Ok(out)
